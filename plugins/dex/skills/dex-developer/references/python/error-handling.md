...
Catch precise exceptions such as `FlowNotFoundError`, `FlowNotActiveError`, `FlowAlreadyStartedError`, `LongPollTimeoutError`, `FlowUncompletedError`, `RpcLockConflictError`, and not-loaded errors. Do not parse messages. Let unexpected defects retain tracebacks.
...
Use automatic failure policies for deterministic recovery, operator Channels for manual choice, and the timeout handler for deadline semantics. Never treat a long-poll timeout as Flow failure.
