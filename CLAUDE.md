...
- `dex-connector-contributor` also loads `dex-sdk` Core and Go guidance plus the
  shared Dex Web v2 reference. It owns connector-library contribution workflow,
  while `dex-app-builder` owns application use of released connectors.
...
`TEMPLATE_BASELINE`. Validate Connector Contributor excerpts against
`CONNECTOR_LIBRARY_BASELINE`. Dex Web is embedded in the Server and CLI
releases.
